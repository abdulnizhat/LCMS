            ...
            if (!IsPostBack)
            {
                try
                {
                    if (Request.QueryString["gaugeId"] != null)
                    {
                        ...
                        ds1 = g.ReturnData1("Select cb.calibration_transaction_id, cb.calibration_schedule_id, DATE_FORMAT(cb.calibration_date, '%d/%m/%Y') as calibration_date, cb.calibration_cost, cb.calibration_hours,cb.calibration_status,cb.certification_no,cb.humidity,cb.other,cb.pressure,cb.temprature,cb.tollerance_go,cb.tollerance_no_go, (Select SUM(calibration_cost) from calibration_transaction_TB  where gauge_id='" + gaugeId + "') as calibCost, gt.gauge_sr_no  from calibration_transaction_TB as cb Left Outer join gaugeMaster_TB as gt ON cb.gauge_id=gt.gauge_id where  cb.status=1 and cb.gauge_id='" + gaugeId + "' Order by cb.calibration_transaction_id DESC ");
