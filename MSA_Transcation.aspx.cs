...
public partial class MSA_Transcation : System.Web.UI.Page
{
    ...
    private void fillGauge(int custId)
    {
        try
        {
            DataTable dtCalibSchedule = g.ReturnData("SELECT t0.gauge_id,concat_WS(': ID- ', t0.gauge_name, CAST(t0.gauge_id as CHAR(20))) AS gauge_name FROM gaugeMaster_TB AS t0 INNER JOIN msa_schedule_TB AS t1 ON t0.gauge_id = t1.gauge_id WHERE (t0.customer_id = " + custId + ") AND (t1.status = True)");
    ...
    private void displayMsaDueStatus()
    {
        try
        {
            string strQuery = " Select cs.msa_schedule_id, cs.gauge_id, gm.gauge_sr_no, gm.gauge_name, gm.gauge_sr_no, gm.gauge_Manufature_Id, gm.gauge_type, cs.next_due_date " +
