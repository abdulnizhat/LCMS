...
public partial class ImportExcel : System.Web.UI.Page
{
                ...
                if (addColumns != null)
                {
                    ...
                    if (formName == "Gauge Part")
                    {
                        try
                        {
                            for (int i = 0; i < Exceldt.Rows.Count; i++)
                            {
                                // DataTable dtsave = g.ReturnData("Insert into gauge_supplier_link_TB (customer_id,gauge_id,supplier_id,link_status,status,linked_date,created_by_id) Values(" + Convert.ToInt32(Exceldt.Rows[i]["customer_id"].ToString()) + ", " + Convert.ToInt32(Exceldt.Rows[i]["gauge_id"].ToString()) + "," + Convert.ToInt32(Exceldt.Rows[i]["supplier_id"].ToString()) + ",'" + Exceldt.Rows[i]["link_status"].ToString() + "', '" + Exceldt.Rows[i]["status"].ToString() + "',  '" + Exceldt.Rows[i]["linked_date"].ToString() + "'," + Convert.ToInt32(Exceldt.Rows[i]["created_by_id"].ToString()) + ")");
    ...
    private string checkdate(DataTable Exceldt)
    {
        ...
        else if (formName == "Calibration Schedule")
        {
            ...
            for (int i = 0; i < Exceldt.Rows.Count; i++)
            {
                if (String.IsNullOrEmpty(Exceldt.Rows[i]["next_due_date"].ToString()))
                {
                    return stmsg = "next_due_date is empty.Date format should be (YYYY-MM-DD) on row position " + i;
                }
                else if (!String.IsNullOrEmpty(Exceldt.Rows[i]["next_due_date"].ToString()))
                {
                    try
                    {
                        DateTime dtdate = Convert.ToDateTime(Exceldt.Rows[i]["next_due_date"].ToString());

                        stmsg = "OK";
                    }
                    catch (Exception)
                    {
                        return stmsg = "next_due_date is not in correct format.Date format should be (YYYY-MM-DD) on row position " + i;
                    }
                }
            }
            ...
            for (int i = 0; i < Exceldt.Rows.Count; i++)
            {
                if (String.IsNullOrEmpty(Exceldt.Rows[i]["projected_calib_schedule"].ToString()))
                {
                    return stmsg = "projected_calib_schedule date is empty.Date format should be (YYYY-MM-DD) on row position " + i;
                }
                else if (!String.IsNullOrEmpty(Exceldt.Rows[i]["projected_calib_schedule"].ToString()))
                {
                    try
                    {
                        DateTime dtdate = Convert.ToDateTime(Exceldt.Rows[i]["projected_calib_schedule"].ToString());

                        stmsg = "OK";
                    }
                    catch (Exception)
                    {
                        return stmsg = "projected_calib_schedule date is not in correct format.Date format should be (YYYY-MM-DD) on row position " + i;
                    }
                }
            }
