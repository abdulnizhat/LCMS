            ...
            if (Status == true)
            {
                ...
                for (int s = 0; s < Dtparent.Rows.Count; s++)
                {
                    ...
                    for (int i = 0; i < dtsubdata.Rows.Count; i++)
                    {
                        ...
                        litMenu.Text = litMenu.Text + "<li ><a href='" + dtsubdata.Rows[i]["navigateurl"].ToString() + "'><span style='color:white; font-size:larger'>" + dtsubdata.Rows[i]["childnode"].ToString() + "</span></a></li>";
